...
これらの情報は、テーブル形式で表示されている。弱点・抵抗力は**タイプ（アイコン）と数値**の組み合わせで、セル内に `span.icon.icon-{タイプ}` と倍率/数値（例: 雷×2、闘－30）が含まれる。

- **弱点**: タイプ（例: 雷 = lightning）＋ 倍率（例: ×2）
- **抵抗力**: タイプ（例: 闘 = fighting）＋ 減少値（例: －30）
- タイプはワザのエネルギーコストと同様に `icon-grass` / `icon-lightning` 等のクラスから取得する。

```javascript
// セルからタイプ（icon クラス）と数値部分を分離して取得する例
function parseCell(cell) {
  ...
  if (typeIcon) {
    ...
    if (typeMatch) type = typeMatch[1];  // grass, lightning, fighting 等
  }
